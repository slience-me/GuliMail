package cn.slienceme.gulimall.seckill.service.impl;
...
import cn.slienceme.gulimall.seckill.feign.CouponFeignService;
import cn.slienceme.gulimall.seckill.feign.ProductFeignService;
        ...
        sessions.stream().forEach(session -> {
            ...
            if (!redisTemplate.hasKey(key)) {
                List<String> values = session.getRelations().stream()
                        ...
                        .collect(Collectors.toList());
                // leftPushAll: 将所有指定的值插入存储在键的列表的头部。如果键不存在，则在执行推送操作之前将其创建为空列表。
